...
type Config struct {
	...
	// CodexPlanRouting forces matching request models to use a specific Codex OAuth plan
	// (for example "free" or "plus") when multiple Codex auth files expose the same model.
	CodexPlanRouting []CodexPlanRoute `yaml:"codex-plan-routing,omitempty" json:"codex-plan-routing,omitempty"`
...
// CodexPlanRoute binds one or more model patterns to a required Codex OAuth plan.
// Patterns follow the same wildcard rules as excluded-models: '*' matches any substring.
type CodexPlanRoute struct {
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Normalize Codex model -> plan routing rules.
...
// SanitizeCodexPlanRouting normalizes model patterns and plan names, and drops empty rules.
func (cfg *Config) SanitizeCodexPlanRouting() {
	if cfg == nil || len(cfg.CodexPlanRouting) == 0 {
		return
	}
	out := cfg.CodexPlanRouting[:0]
	for i := range cfg.CodexPlanRouting {
		entry := cfg.CodexPlanRouting[i]
		entry.Models = NormalizeExcludedModels(entry.Models)
		entry.Plan = strings.ToLower(strings.TrimSpace(entry.Plan))
		if len(entry.Models) == 0 || entry.Plan == "" {
			continue
		}
		out = append(out, entry)
	}
	cfg.CodexPlanRouting = out
}
